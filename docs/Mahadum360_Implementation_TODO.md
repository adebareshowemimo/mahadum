...
- [x] 🟡 **WCAG 2.1 AA** — automated **axe** checks in CI (`npm test`) over login/register/forgot/dashboard + modal dialog semantics — all clean. **Manual audit, 2026-07-07 (partial):** fixed a real nested-interactive-elements bug (`<Link><Button>` → `<a><button>`, invalid HTML/double-announced to screen readers) across all 16 occurrences via a new `LinkButton` component; fixed a real color-contrast failure (`--subtle` measured 3.64–3.68:1 against `--background` in both themes, below the 4.5:1 AA minimum — verified programmatically, not by eye, then re-verified after the fix). *(Still ⬜: tap targets ≥44px — some icon-only buttons measured 32–36px, a self-imposed mobile-usability bar since WCAG 2.1 AA itself has no target-size criterion; full screen-reader walkthrough beyond the pages axe already covers; 200% zoom; captions — needs a browser-based pass.)*
