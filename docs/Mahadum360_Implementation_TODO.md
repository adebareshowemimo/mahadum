...
| 6 | Monetisation & billing (cards + telco) | 5 | ✅ backend + frontend complete (paywall/entitlements/subs/telco-opt-in/data-bundles/ad-network seam/ad screen/post-lesson interstitial); no real ad vendor chosen yet (`NullAdGateway`) |
...
> 📋 **Recommendations drafted for every open item below: `Mahadum360_Open_Decisions.md`** (2026-07-18).
...
- [x] 🟡 Sign-up flow + account type + age gate + parent/child branch (COPPA). *(multi-step interactive wizard: **account type** (`Family`, `Educator/School`, or `Institution`) → **DOB age-gate** → digital-age check from `GET /config` (`digital_age` = admin `COMPLIANCE_MINOR_AGE`); **under-age → guardian-setup branch** w/ parental-consent checkbox; adults go straight to the form. Family signup creates a parent-owned family; Educator/School and Institution signups create a pending organization with the registrant as `school_admin`, ready for platform verification. Class-invitation registration remains a learner account. The details step separates **Email & password** from **Google** signup: Google supplies the verified name/email while MAHADUM.360 collects and persists phone, DOB, account type, and organization name without asking for a password. Remaining: actual child-profile creation from dashboard + server-side consent record for the unauth under-age path ⬜)*
...
- [x] ✅ Parent course discovery without profile selection. *(`/learn` and `/learn/courses` fetch and display every published course before a learner is active; enrollment/progress actions remain disabled and explain how to select or add a learner. The parent dashboard links directly to the catalogue. Selecting a learner changes `/learn` back to that learner's journey and enriches the catalogue with enrollment state.)*
- [x] ✅ Cross-platform coin icon. *(replaced the font-dependent `🪙`/currency glyphs with the shared in-house SVG `Icon name="coin"` across learner stats, family, wallet, reviews, rewards, assignments, and marketing surfaces so Windows and mobile cannot render a missing-glyph square.)*
- [x] ✅ Admin-managed learning XP defaults. *(quiz completion defaults to 1 XP and video completion to 5 XP; super-admin System settings can change both, with audited updates synchronized to existing and newly authored components.)*
- [x] ✅ Learner profile pictures. *(learners and parents can choose from eight built-in avatars or upload a validated JPG/PNG/WebP photo; updates are ownership-protected, audited, and shown throughout family and learning profile surfaces.)*
- [x] ✅ Parent self-learning. *(parents can start any published course from `/learn/courses` with their own account; the platform creates/reuses a separate adult learner identity so enrollment, progress, XP, avatars, and child PIN boundaries remain intact.)*
- [x] ✅ Scalable learner course library. *(`/learn/courses` uses server-side title/topic search, language and level filters, 12-item pagination, result counts, compact rows, and explicit parent-self/child context. `/learn` remains a focused journey and links to discovery instead of loading the entire catalogue.)*

**QA**
- [x] ✅ Auth flows (register/login/google/reset) feature tests (2026-07-18) — register/login were already covered by `AuthTest`; added `GoogleAuthTest` (6 tests: first-login creates a pre-verified parent + family, links an existing password account by email, repeat login reuses the account, unverifiable token → 401, validation, and an OAuth-only account cannot be logged into by password) and `PasswordResetTest` (5 tests: reset link sent, no account enumeration on unknown email, reset revokes all tokens, invalid token → 422, confirmation required). **Found and fixed a production bug in the process:** `users.password` was `NOT NULL`, so `AuthController@google` — which creates a user with only a `google_id` — 500'd on *every* first-time Google sign-up. Migration `2026_07_18_000001_make_users_password_nullable` makes it nullable, and `login()` now refuses a null password outright so an OAuth-only row can never be reached by password.
- [x] ✅ **Tenant isolation tests** (School A cannot read School B) (2026-07-18) — `TenantIsolationTest`. 403-on-access-by-id was already covered across `SchoolAndAdminTest`/`SchoolReferralTest`/`InvoicePdfTest`; the untested gap was **list leakage** (a broken global scope wouldn't 403, it would silently include foreign rows). Now asserts the class list and invoice list exclude another tenant's rows, and that super_admin runs unscoped. Mutation-verified: disabling `TenantScope` makes the class-list test fail. *(Note: the invoice list survives that mutation — it is scoped by the route's `{organization}` param + policy rather than the global scope, so the two layers are independently enforced.)*
...
- [x] ✅ Content-owner dashboard visibility — `/home` shows performance per owned course: subscription status mix, referral users/revenue, pending unconfirmed + escrow earnings, and attributed revenue, with search, status filtering, and pagination. The same report remains available at `/content/performance`.
...
- [x] ✅ **Monnify checkout + super-admin configuration**: two-step authenticated hosted checkout, signed/idempotent webhooks and refund correlation; `/admin/settings/gateways` securely manages encrypted credentials, sandbox/live environment and checkout enablement, with a harmless connection test. Environment values remain fallbacks and secrets are never returned or audited.
...
- [x] ✅ **Banner advert coverage** — free parent/student/supervisor sessions retain the global leaderboard and now receive the managed inline placement on home, learning, course catalogue, family, learner profiles, achievements, leaderboard, competitions, and referrals. Premium users and staff remain ad-free; lesson playback, billing, wallet, reviews, support, authoring, and admin flows are deliberately excluded.
- [x] ✅ Receipts: `SubscriptionActivated` notification on activation, delivered over email + push + SMS/WhatsApp.
- [x] ✅ **Email delivery configuration**: `/admin/emails/configuration` exposes the real mailer/queue state, stores SMTP credentials encrypted, manages sender identity, and sends an immediate branded test email. Queue workers reload integration settings before every job.
...
- [x] ✅ **Ad screen** (2026-07-07) — `AdModal` (`web/src/components/gamification/AdModal.tsx`): placement-agnostic (skip timer → verified completion → reward), a COPPA/unavailable **fail fallback** (never dead-ends — Rule 4), and a **remove-ads upsell** link to `/billing`. Wired into the `rewarded_heart` flow on `/achievements` ("Watch ad" now actually shows one instead of instantly granting the refill). **`post_lesson` wired 2026-07-18** — `LessonComplete` (`LessonPlayerPage`) opens the interstitial when the learner taps "Back to journey", i.e. only after the lesson has finished and only on the way out, satisfying Rule 10 (never mid-lesson). Gated on the `ads` entitlement so paid tiers never see it — unlike `rewarded_heart`, which is user-initiated and therefore ungated. Closing the modal always continues to the journey, so a COPPA-blocked or unavailable ad can never strand the learner. Frequency capping is deliberately left to the gateway's `available()` seam rather than hardcoded in the client. Verified live end-to-end: request → play → complete → hearts refilled → impression row shows `shown_at`/`consumed_at` set.
...
- [x] ✅ **Activation gate + per-purchase commission model** (2026-09-03) — a code activates only once the referred person has a paid subscription **and** has finished ≥1 lesson + ≥1 quiz (`ReferralService::maybeActivate`, hooked from `LessonCompletionController` / `AnswerController` / `PaymentService`; safety-net `referrals:sync-activations`). After activation, **every** purchase (subscription charges + wallet funding) within a 30-day window earns the code owner 5% into escrow (`recordReferredPurchase`, idempotent on `source_event`; `reverseForSource` clawback). **Explicit invites**: `referral_invitations` (email/phone), matched at sign-up; inviting an already-active account → `422 account_exists`. **All rules are admin-editable** platform settings (`config/settings.php` "referrals" group): `commission_bps`, `earning_window_days`, `escrow_days`, `velocity_limit`, `activation_min_lessons`, `activation_min_quizzes`, `activation_requires_paid_subscription`.
...
- [x] ✅ **Invite + activations dashboard** (2026-09-03) — `/referrals` gains an "Invite a friend" card (email/phone, shows the "account already exists" prompt) and an Activations table (SN · date · code · via email · via phone · Active/Inactive) searchable by email/phone (`GET /referrals/activations`, `GET|POST /referrals/invitations`). Super-admin per-code dashboard at `/admin/referrals` (`ReferralCodesAdminPage`, `GET /admin/referrals/codes`): count activated, via-email / via-phone split, active/inactive counts. Referral knobs surface automatically in `/admin/settings`.
...
- [x] ✅ Roster + CSV import; classes; seats; invoices — `/roster` (**CSV upload** → per-row import w/ error report), `/seats` (filled gauge + **buy seats** with volume discount + proforma invoice), `/invoices` (itemized **Student School Fees** and **Registration Fees**, VAT and total + **PDF download**). Registration remains visible as ₦0 when waived for a top-up. Verified live (60 seats → invoice; CSV 2 imported + bad row flagged; PDF stream).
- [x] 🟡 Teacher portal: classes + earnings + referral hub — `/classes` (teacher's classes → student roster modal, `GET /classes` + `/classes/{id}`), `/earnings` (cleared vs escrow commissions + payouts + request payout), referral hub reused at `/referrals`. Verified live (teacher Angelica: 2 classes, 11/18 students). **Teacher analytics ✅ (2026-07-03):** `GET /classes/{class}/analytics` (`SchoolClassController@analytics`, `can:view,class`) — per-student lessons-completed, avg lesson score, quiz accuracy, speaking count; surfaced as a **Students / Analytics** tab in the class modal (`ClassesPage`). Verified live. **Class-level assignments ✅** — shipped as `ClassAssignment` (a distinct model from the content-component-level `assignments` table), with create/list/show/submit/grade under `classes/{class}/assignments`, completion surfaced in class analytics, and a real `/assignments` page. `ComingSoon` is now unreachable: every one of the 45 nav destinations resolves to a built page.
...
- [x] 🟡 Security hardening landed: payout SoD, signed+idempotent webhooks (payment **and** telco), refund/chargeback reversal + commission clawback, telco OTP, SPA CSRF auth, COPPA/NDPA consent, audit logging, rate limiting on auth/OTP. **2026-07-07: scoped review of the school-referral-hub + ad-network diffs (identify → independently verify each finding) found and fixed one real authz gap** — `AdController`/ad-based `HeartController::refill` let any same-tenant staff member (not just the parent) trigger/redeem ad rewards for an arbitrary learner via the `learning.progress.view` policy branch; added `LearnerProfilePolicy::redeemReward` (self/parent only) + regression test. This was a scoped diff review, not a full-codebase audit. **External penetration test + a full-codebase formal authz audit still ⬜.**
- [ ] 🟡 Load test billing concurrency + leaderboard; performance budget (<3s on 3G). *(Load testing still ⬜.)* **Bundle budget ✅ (2026-07-18):** the SPA shipped as one 758 kB (201 kB gzip) chunk — every learner downloaded all 30 admin routes. Now route-level `React.lazy` (~96 chunks) + `manualChunks` splitting `vendor-react` / `vendor-data`. Entry 758 kB → **95 kB**; cold critical path **~355 kB / ~112 kB gzip** (entry + both vendors) vs 201 kB gzip before, and app-code deploys no longer bust the vendor cache. Two Suspense boundaries, not one: the inner boundary is nested under `AppLayout` so in-app navigation doesn't unmount and flash the whole shell.
- [x] 🟡 **WCAG 2.1 AA** — automated **axe** checks in CI (`npm test`) over login/register/forgot/dashboard + modal dialog semantics — all clean. **Manual audit, 2026-07-07 (partial):** fixed a real nested-interactive-elements bug (`<Link><Button>` → `<a><button>`, invalid HTML/double-announced to screen readers) across all 16 occurrences via a new `LinkButton` component; fixed a real color-contrast failure (`--subtle` measured 3.64–3.68:1 against `--background` in both themes, below the 4.5:1 AA minimum — verified programmatically, not by eye, then re-verified after the fix). **Tap targets ✅ (2026-07-18):** new shared `IconButton` primitive (`web/src/components/ui/IconButton.tsx`) guarantees a ≥44×44 tappable region while keeping the *painted* box at its original size, via a centred transparent `::after` overlay — so the Gilded Adire visual language is unchanged. Applied to `Modal` close, `AppLayout` (close-menu / hamburger / theme toggle / profile trigger), `ProfileSwitcher`, `SlideDeck` exit, and the `Button`/`Button3D` `sm` variants + `Switch`. Four `LessonBuilderPage` controls were **visually** enlarged to 44 instead, because they sit flush with zero gap and overlapping invisible overlays would fight each other. Nav and dropdown items got `min-h-11`. *(Known residual: `AppLayout`'s `OrgSwitcher` native `<select>` is 36px — a native select can't take the overlay trick, so raising it is a real visual change; it's `hidden sm:flex` so it never renders on a phone in portrait, but it does still fail ≥640px.)* *(Still ⬜: full screen-reader walkthrough beyond the pages axe already covers; 200% zoom; captions — needs a browser-based pass.)*
