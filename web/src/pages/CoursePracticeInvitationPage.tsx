...
import { ApiError, learningApi, type CoursePracticeInvitation } from '@/lib/api'

export function CoursePracticeInvitationPage() {
  const { token = '' } = useParams()
  const [invitation, setInvitation] = useState<CoursePracticeInvitation | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)

  useEffect(() => {
    learningApi.coursePracticeInvitation(token)
      .then(setInvitation)
  ...
  async function accept() {
    setBusy(true)
    try {
      setInvitation(await learningApi.acceptCoursePracticeInvitation(token))
    } catch (err) {
    ...
    <div className="mx-auto flex min-h-screen max-w-xl items-center p-6">
      <Card className="w-full">
        <CardBody className="flex flex-col gap-5 p-6 text-center">
          ...
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground">Practice-with-me invitation</h1>
            <p className="mt-2 text-muted">{invitation.inviter_name} invited you to practice “{invitation.lesson_title}” together.</p>
            {invitation.course_title && <p className="mt-1 text-sm text-subtle">Part of {invitation.course_title}</p>}
          </div>
          <p className="text-xs text-muted">No child contact details or internal learner identifiers are shared.</p>
          {invitation.accepted
            ? <Alert variant="success">Accepted — you can now practice this lesson together.</Alert>
