...
type SignupAccountType = 'family' | 'educator_school' | 'institution'
...
const ACCOUNT_TYPES: Array<{
...
}> = [
  {
    value: 'family',
    label: 'Family',
    description: 'Learn yourself or create safe profiles for children.',
    icon: 'users',
  },
  {
...
export function RegisterPage() {
  ...
  function continueFromAge(e: FormEvent) {
    ...
    if (age < digitalAge) {
      if (accountType && accountType !== 'family') {
        setAgeError(`You must be at least ${digitalAge} to create an Educator/School or Institution account.`)
        return
      }
      if (invitationToken) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const payload: RegisterInput = {
        ...
        ...(!invitationToken && accountType !== 'family'
      ...
      navigate(invitationToken ? '/learn' : accountType === 'family' ? '/home' : '/school', { replace: true })
    } catch (err) {
  ...
  function validateGoogleSignup(): boolean {
    ...
    if (accountType !== 'family' && !values.organization_name.trim()) {
            ...
            {(signupMethod === 'email' || invitationToken) ? (
              ...
              <GoogleButton
                ...
                signupContext={{
                  ...
                  ...(!isGuardianFlow && dob ? { date_of_birth: dob } : {}),
                  ...(accountType !== 'family' ? { organization_name: values.organization_name.trim() } : {}),
                  ...(referralCode ? { referral_code: referralCode } : {}),
                }}
                onSuccess={() => navigate(accountType === 'family' ? '/home' : '/school', { replace: true })}
