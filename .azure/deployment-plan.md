...
### Folder-aware media storage import validation — 2026-09-21

- [x] Releases `3e0759a` and `5f1ee17` are pushed to `origin/codex/beta-feedback-20260903`; the importer keys assets by full storage path and can be restricted to explicit media folders.
- [x] Production read-only inspection found 45 Yoruba, 37 Igbo, and 44 Hausa files (126 total), while `media_assets` had 79 rows and zero foldered rows. None of the target subfolder paths had database records.
- [x] Final backend validation passed: 361 tests, one existing skip, 1,844 assertions; focused storage/media coverage passed, including identical filenames across folders, idempotency, metadata backfill, dry-run, and folder scoping. Pint and PHPStan level 5 passed.
- [x] Azure CLI confirmed subscription `4212afa5-d96d-4717-a56d-1d34956599a6`, running VM `mahadum` in Canada Central at `20.151.177.171`, unchanged Security Center policy, 98 GB free disk, healthy runtime services, and HTTPS health returning 200.
- [x] The update has no schema, dependency, infrastructure, RBAC, or file-content changes. Production import is additive and limited to the three named folders; video files are never modified or deleted.

Deployment will publish the immutable source commit with a verified source/database backup, run an explicit three-folder dry-run, execute the same scoped import only if the preview reports the expected 126 paths, then verify per-folder database/API totals and public health. Rollback will restore source and database without touching the video files.

**Deployment result:** Release `5f1ee17` deployed successfully at `2026-09-22T00:11:59Z`. The scoped dry run reported exactly 126 creates and no updates, unchanged files, or unsupported files; the production import then created 45 Yoruba, 37 Igbo, and 44 Hausa media rows without changing video files. Forty-one filenames occur in more than one language folder and now coexist as distinct full-path assets. Authenticated browser verification showed 205 total assets, separate folder buttons/counts, and a Yoruba filter returning 45 correctly labelled rows. Apache, PHP-FPM, MySQL, and the queue are active; public `/`, `/up`, `/media`, and `/api/v1/config` returned HTTP 200, with no post-deployment application errors. Immutable archive SHA-256 `51ea0e6d31bac86b2f10228ee2f6ed2f8076b5cf28d64a3044beae386326800d`. Backup `/var/backups/mahadum/media-sync-5f1ee17-20260922T001134Z`: source SHA-256 `b411f6ab3c961263c73da7c4c68e1fc086f7a2319dc20d6b39c39a4858b1deca`; database SHA-256 `6e6a3a7bf5bbf4d1f6af0da34440d2f74b6474f5f1bca519be216b929feb8f39`.
