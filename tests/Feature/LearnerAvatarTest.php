...
class LearnerAvatarTest extends TestCase
{
    ...
    public function test_parent_can_choose_an_avatar_from_the_expanded_library(): void
    {
        $this->seedRbac();
        $parent = $this->actingAsUser($this->userWithRole('parent'));
        $family = Family::create(['owner_user_id' => $parent->id, 'name' => 'Family']);
        $child = LearnerProfile::create(['family_id' => $family->id, 'display_name' => 'Zainab']);

        $this->postJson("/api/v1/learners/{$child->id}/avatar", ['avatar_id' => 54])
            ->assertOk()
            ->assertJsonPath('data.avatar_id', 54);
    }
