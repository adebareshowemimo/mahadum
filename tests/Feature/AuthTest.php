...
class AuthTest extends TestCase
{
    ...
    public function test_individual_registration_alias_creates_a_direct_learner_without_a_family(): void
    {
        $this->seedRbac();

        $response = $this->postJson('/api/v1/auth/register', [
            'first_name' => 'Chika', 'last_name' => 'Nwosu',
            'email' => 'chika.individual@test.local', 'phone' => '+2348012345689',
            'password' => 'Password123!', 'password_confirmation' => 'Password123!',
            'device_name' => 'Laptop', 'account_type' => 'individual', 'date_of_birth' => '1992-06-14',
        ])->assertCreated()->assertJsonPath('data.abilities', ['student']);

        $userId = $response->json('data.user.id');
        $this->assertDatabaseHas('learner_profiles', ['user_id' => $userId, 'display_name' => 'Chika Nwosu']);
